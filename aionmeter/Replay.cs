...
namespace AIONMeter
{
    public class Replay : IDisposable // Replays a log file
    {
        ...
        public Replay(MODE _mode, string replay_file)
        {
            ...
            if (mode == MODE.DEBUG) DebugLog.start();
            if (mode == MODE.IGNORE_COMBAT) Meter.active_meter.parser.mode = Parser.MODE.IGNORE_COMBAT;
        ...
        private void start()
        {
            ...
            if (file_exists)
            {
                while ((line = reader.ReadLine()) != null) // read till end line by line
                {
                    ...
                    if (DebugLog.on) DebugLog.write_line("[" + line + "]");
        ...
        public void count_lines()
        {
            ...
            if (DebugLog.on) DebugLog.write_line("Input file has a total of " + all_lines.Length + " lines.");
